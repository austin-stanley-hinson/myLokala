...
type BusinessQrPanelProps = {
  paymentUrl: string;
  businessName: string;
};

/**
 * Client-only QR rendering + copy/download/print helpers.
 * Ownership and get-or-create stay on the server; this only visualizes the URL.
 */
export function BusinessQrPanel({
  paymentUrl,
  businessName,
}: BusinessQrPanelProps) {
    ...
    <div className="mt-5 grid gap-6 rounded-3xl border border-lokala-border bg-white p-6 shadow-lokala-card sm:grid-cols-[auto_1fr] sm:items-start">
      ...
      <div className="min-w-0 space-y-4">
        ...
        <ul className="space-y-1.5 text-sm leading-6 text-lokala-muted">
          ...
          <li>
            Customers scan it, enter their amount, add a tip, and show
            confirmation to staff.
          </li>
          <li>
            Your staff do not need to download Lokala or use a personal phone.
          </li>
        </ul>
      </div>
    </div>
